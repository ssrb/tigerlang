        ...
        let rewriteProgram' asm (spill : Liveness.Assem.Variable.t) = 
            ...
            let rewriteOperands fs ops  = 
                if List.exists ~f:(fun (o : Assem.Variable.t) -> o.temp = spill.temp) ops then
                    ...
                    let ttmp = { t = TEMP t; addr = spill.regclass = "a" } in
                ...
                | A.OPER op ->
                 ...
                 | A.MOVE mv ->
                    ...
                    begin
                        ...
                        | (true, true) -> []
                        | (true, _) -> Codegen.codegen frame (MOVE ({ t = TEMP mv.dst.temp; addr = mv.dst.regclass = "a" }, memory))
                        | (_, true) -> Codegen.codegen frame (MOVE (memory, { t = TEMP mv.src.temp; addr = mv.src.regclass = "a" }))
                        | _ -> [ A.MOVE mv ]
                    end
