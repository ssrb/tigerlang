...
type decl_ast = START of string
			| TERM of (string * string option) list
			| TERMPREFIX of string
			| RULEPREFIX of string
			| SIG of string

type pattern_ast = PAT of (string * pattern_ast list)

type rule_ast = RULE of (string * pattern_ast * string * int list)

type spec_ast = SPEC of {
	head : string list;
	decls : decl_ast list; 
	rules : rule_ast list;
	tail : string list
